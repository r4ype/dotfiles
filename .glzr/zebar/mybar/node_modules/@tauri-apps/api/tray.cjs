...
function mapEvent(e) {
    const out = e;
    out.position = new dpi.PhysicalPosition(e.position.x, e.position.y);
    out.rect.position = new dpi.PhysicalPosition(e.rect.position.Physical.x, e.rect.position.Physical.y);
    out.rect.size = new dpi.PhysicalSize(e.rect.size.Physical.width, e.rect.size.Physical.height);
    return out;
}
