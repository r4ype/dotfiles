...
class Window {
    ...
    async innerPosition() {
        return core.invoke('plugin:window|inner_position', {
            label: this.label
        }).then(({ x, y }) => new dpi.PhysicalPosition(x, y));
    }
    /**
    ...
    async innerSize() {
        return core.invoke('plugin:window|inner_size', {
            label: this.label
        }).then(({ width, height }) => new dpi.PhysicalSize(width, height));
    }
    /**
...
function mapPhysicalPosition(m) {
    return new dpi.PhysicalPosition(m.x, m.y);
}
function mapPhysicalSize(m) {
    return new dpi.PhysicalSize(m.width, m.height);
}
/**
...
async function cursorPosition() {
    return core.invoke('plugin:window|cursor_position').then(mapPhysicalPosition);
}
